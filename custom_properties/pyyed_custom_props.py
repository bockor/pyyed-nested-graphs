...
#&
custom_property_scopes = ['node', 'edge', 'graph']

#&
class CustomProperty():

    def __init__(self, scope, name, property_type): 
        '''
        scope: [node|edge|graph]
        name: name of the custom property
        property_type: according yEd -> Text (string CONFIRMED), Binary (boolean CONFIRMED), Integer, Decimal, XML
        '''
        self.scope = scope
        self.name = name
...
class Node:
    ...
    def __init__(self, node_name, label=None, label_alignment="center", shape="rectangle", font_family="Dialog",
        ...
        if custom_properties:
            for key in custom_properties:
                if key in Node.custom_properties_names:
                    setattr(self, key, custom_properties[key])
                else:
                    raise RuntimeWarning("key %s not recognised" % key)
    ...
    def convert(self):
        ...
        #& Node Custom Properties
        for key in Node.custom_properties_names:
            node_custom_prop = ET.SubElement(node, "data", key=Node.custom_properties_names[key])
            node_custom_prop.text = getattr(self, key, "Not assigned")

        return node

    #&
    @classmethod
    def set_custom_properties_names(cls, custom_property):
        cls.custom_properties_names.update({custom_property.name: custom_property.id })
        print(cls.custom_properties_names)
...
class Edge:
    ...
    def __init__(self, node1, node2, label="", arrowhead="standard", arrowfoot="none",
        ...
        if custom_properties:
            for key in custom_properties:
                if key in Edge.custom_properties_names:
                    setattr(self, key, custom_properties[key])
                else:
                    raise RuntimeWarning("key %s not recognised" % key)
    ...
    def convert(self):
        ...
        #& Edge Custom Properties
        for key in Edge.custom_properties_names:
            node_custom_prop = ET.SubElement(edge, "data", key=Edge.custom_properties_names[key])
            node_custom_prop.text = getattr(self, key, "Not assigned")
...
class Graph:
    #&
    def __init__(self, directed="directed", graph_id="G", graph_custom_properties=None):
    ...
    #&
    def add_custom_property(self, scope, name, property_type):
        if scope not in custom_property_scopes:
            raise RuntimeWarning("Scope %s not recognised" % scope)
        custom_property = CustomProperty(scope, name, property_type)
        self.custom_properties.append(custom_property)
        if scope == "node":
            Node.set_custom_properties_names(custom_property)
        elif scope == "edge":
            Edge.set_custom_properties_names(custom_property)
        elif scope == "graph":
            Graph.set_graph_custom_properties_names(custom_property)
